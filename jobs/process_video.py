...
async def _process_video_async(video_id: UUID) -> None:
    ...
    from utils.moments import generate_moments_for_video
    ...
    try:
        ...
        # Section 4.1 - Extract audio (outside DB session)
        audio_path = await extract_audio_async(video_path)
        
        # Section 4.2 - Transcribe audio (outside DB session)
        transcript_segments = await transcribe_audio_async(audio_path)
        
        # Save transcript in new session
        async with async_session_maker() as db:
        ...
        # 5.3 - Find highlights using GPT (parallel processing, outside DB session)
                ...
                try:
                    async with async_session_maker() as db:
                        highlight_db = await crud.create_highlight(
                            db, video_id, highlight.start, highlight.end, highlight.score, highlight.title, highlight.summary, prompt_version_id
                        )
                        saved_count += 1
        ...
        # Section 6 - Generate moments (DISABLED - moments are no longer generated as MP4 files)
        # Moments are now virtual references to highlight timestamps only
        logger.info(f"[ProcessVideo] Moment generation disabled - using virtual moments based on highlights")
        
        # Update status to DONE
        async with async_session_maker() as db:
            await crud.update_video_status(db, video_id, VideoStatus.DONE)
            logger.info(f"[ProcessVideo] ✅ Completed processing video: {video_id} - Status set to DONE")
